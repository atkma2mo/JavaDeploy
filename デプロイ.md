...
Jenkins CircleCIなどは使いません。学内Webサーバへのデプロイ手順を説明します。クラウドの話もでません。
...
# Openjdk11 maven h2DB nginx(リバースプロキシサーバ)をインストール  
...
↑Apache、Tomcatの連携を行う必要がある。Apacheを除いたTomcat単体でもWebサーバとしての機能を確保している。Tomcatだけでサーバ処理を行えばシンプルだが、Apacheと比較すれば性能は劣るため、2つを連携させることでより優位なWebサーバ構築が可能となる。(Apache Tomcat連携で調べる。)  
...
h2DBはPureJavaのため事前にjdkのインストールが必要です。先ほどインストールしたので大丈夫。    
